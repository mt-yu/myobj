...
def article_detail(request, id, slug):
    ...
    total_views = r.incr("article:{}:views".format(article.id))  # 对访问文章的次数进行记录，incr函数是递增

    r.zincrby('article_ranking', 1, article.id)  # zincrby(name, value, amount) redis数据库方法

    article_ranking = r.zrange("article_ranking", 0, -1, desc=True)[:10]  # zrange(name, start) redis数据库方法 找到排序前十的
    article_ranking_ids = [int(id) for id in article_ranking]
    most_viewed = list(ArticlePost.objects.filter(id__in=article_ranking_ids))  # 得到最热文章组成的列表
    most_viewed.sort(key=lambda x: article_ranking_ids.index(x.id))  # 对得到的热文列表进行排序
